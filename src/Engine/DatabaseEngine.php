...
use Crustum\Explorator\Engine\Trait\PaginatesDatabaseVectorSearchTrait;
use Crustum\Explorator\Engine\Trait\PerformsDatabaseVectorSearchTrait;
use ReflectionMethod;

/**
 * Database LIKE / full-text search engine.
 */
class DatabaseEngine extends Engine implements SupportsSemanticSearch
{
    use PaginatesDatabaseVectorSearchTrait;
    use PerformsDatabaseVectorSearchTrait;
    ...
    {
        $this->updateSearchableEmbeddings($entities);
    }
    ...
    public function search(Builder $builder): mixed
    {
        ...
        if ($builder->semanticSearch) {
            $entities = $this->buildSemanticSearchQuery($builder)->all()->toList();
    ...
    {
        if ($this->shouldPerformHybridSearch($builder)) {
            return $this->paginateHybridSearch($builder, $perPage, 'page', $page);
        }

        if ($builder->semanticSearch) {
            return $this->paginateSemanticSearch($builder, $perPage, 'page', $page);
        }
    ...
    {
        if ($this->shouldPerformHybridSearch($builder)) {
            return $this->simplePaginateHybridSearch($builder, $perPage, 'page', $page);
        }

        if ($builder->semanticSearch) {
            return $this->simplePaginateSemanticSearch($builder, $perPage, 'page', $page);
        }
